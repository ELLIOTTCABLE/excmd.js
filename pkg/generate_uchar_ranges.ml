...
let dump_header () =
   print_endline
      "(* The following matchers are generated by `pkg/generate_uchar_ranges.ml`, and \
       then prepended to" ;
   print_endline
      " * `src/lexer.body.ml` to produce `src/lexer.ml`. They can not be modified \
       directly! *)" ;
   print_endline ""
...
let () =
   ...
   try
      ...
      do
         ...
         and combining_class =
            get_exn (Uucd.cp_prop ucd i Uucd.canonical_combining_class)
         in
         ( match (general_category, combining_class) with
